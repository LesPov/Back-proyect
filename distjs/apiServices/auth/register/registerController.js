...
const newUser = async (req, res) => {
    try {
        ...
        const existingUserError = await (0, checkExistingUserOrEmail_1.checkExistingUserOrEmail)(username, email);
        ...
        const newUser = await (0, createNewUser_1.createNewUser)(username, hashedPassword, email, rol);
        // Inicializar el perfil de usuario en la base de datos, asignando un ID único
        await (0, initializeUserProfile_1.initializeUserProfile)(newUser.id);
        // Generar y guardar un código de verificación para el correo electrónico del usuario
        const verificationCode = await (0, createVerificationEntry_1.createVerificationEntry)(newUser.id, email);
        // Enviar un correo electrónico de verificación al nuevo usuario con el código generado
        await (0, sendEmailVerificationCode_1.sendVerificationEmail)(email, username, verificationCode);
